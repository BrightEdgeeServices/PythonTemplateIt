...
- Pytest is configured in `pyproject.toml` (`testpaths = tests`, `pythonpath = src,tests`). `tests/legacy/` is
  intentionally ignored. Add tests next to related unit domains (for example, downloader changes ->
  `tests/unit/test_core_downloader.py`).
...
- Do not mock database connections.
- Do not mock Google dependencies e.g., GoogleDriveManager
